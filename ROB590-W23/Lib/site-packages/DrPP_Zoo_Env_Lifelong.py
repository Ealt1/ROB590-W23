...
def action_to_space(action,point):
    ...
    # if NUM_ROWS < my_point_temp_2D[0] or NUM_COLS <my_point_temp_2D[1]:
    #     print("INVALID ACTION:",action,point,my_point_temp_2D)
...
class CustomEnvironment(ParallelEnv):
    ...
    def reset(self, seed=None, return_info=False, options=None):
        ...
        for agent in self.agents:
            valid_actions = calculate_valid_space(self.final_locations[int(agent)],self.current_locations[int(agent)],self.moves_remaining[int(agent)])
    ...
    def step(self, actions):
        ...
        for agent in actions:
            if self.runs_completed[int(agent)]:
                self.runs_completed[int(agent)] = 0
                self.refresh_agents[int(agent)] = 1
                self.my_indexes_movement[int(agent)] = 0
                self.Paths[int(agent)].fill(-1)
                self.initial_locations[int(agent)] = self.final_locations[int(agent)]
            ...
            if np.any(path_temp):
                ...
                self.paths_ready = 0
                self.moves_remaining[int(agent)] = self.moves_remaining[int(agent)] - 1
        paths_temp = np.array(copy(self.Paths))
        if -1 not in paths_temp:
            self.paths_ready = 1
        # print("Moves Remaining:",self.moves_remaining)
        ...
        if self.paths_ready:
            indexes_pre = [-1]*self.N
            mypaths = eng.cell(1,self.N)
            starting_points = [-1]*self.N
            for val in range(self.N):
                if self.refresh_agents[val]:
                    self.time_elapsed[val] = 0
                    curr_pos_idx_field = 1
                    self.refresh_agents[val] = 0
                    ...
                    self.paths_during_movement[val] = mypaths[val]
                    self.my_indexes_movement[val] = 0
                else:
                    # curr_pos_idx_field = int(eng.getfield(self.Agents[val],'curr_pos_idx'))
                    curr_pos_idx_field = self.my_indexes[val]
                    # path_temp = self.paths_during_movement[val]
                    # mypaths[val] = path_temp[int(self.my_indexes_movement[val]):]
                    mypaths[val] = self.paths_during_movement[val][int(self.my_indexes_movement[val]):]
                starting_points[val] = copy(mypaths[val][0])
                indexes_pre[val] = copy(curr_pos_idx_field)
                # print(self.Paths)
            self.myflag = 0
            self.Agents = eng.cell(1,self.N)
            for i in range(0,self.N):
            ...
            mybreak = False
            self.time_limit_elapsed = False
            if not self.myflag:
                self.random_order = eng.randperm(self.N)
                while not self.time_limit_elapsed and not mybreak: # and np.sum(self.runs_completed) < self.N
                    for m in range(0,self.N):
                        ...
                        if not self.runs_completed[n]:
                            ...
                            # while True:
                            #     eng.getfield(self.Agents[n],'curr_pos_idx')
                            #     mypath = eng.getfield(self.Agents[n],'path')
                            #     if mypath[0][int(curr_pos_idx_field)-1] == mypath[0][int(curr_pos_idx_field)]:
                            #         # eng.move_philosopher(self.Agents[n],nargout=0)
                            #         # eng.workspace['myflag']
                            #     else:
                            #         break
                            ...
                            if eng.getfield(self.Agents[n],'curr_pos_idx') == eng.length(eng.getfield(self.Agents[n],'path')):
                                ...
                                self.moves_remaining[n] = MAX_MOVES
                                mybreak = True
                                break
                        self.time_elapsed_total = self.time_elapsed_total+1
                        # print("total time:",self.time_elapsed_total)
                        # threshold_mask = self.time_elapsed[n] > 15
                        if self.time_elapsed[n] > 50:
                            self.time_elapsed[n] = 0
                            self.time_limit_elapsed = True
                    ...
                    for n in range(self.N):
                        ...
                        self.my_indexes[n] = curr_pos_idx_field
                        mypath = eng.getfield(self.Agents[n],'path')

                        # self.positions[n] = self.Agents[n].path(self.Agents[n].curr_pos_idx)
                        self.positions[n] = mypath[0][int(curr_pos_idx_field)-1]
                    # print("Indixes prior to plot:",self.my_indexes)
                    # print("Post positions:",self.positions)
                    eng.plot_ws(self.ws, self.positions, self.final_locations, [],nargout=0)
                    eng.drawnow
                for n in range(self.N):
                    self.my_indexes_movement[n] = self.my_indexes_movement[n] + self.my_indexes[n] - 1
        ...
        observations = {}
        my_current_locations = np.array([0]*self.N)
        for agent in self.agents:
            if self.my_indexes_movement[int(agent)] != 0:
                my_current_locations[int(agent)] = self.paths_during_movement[int(agent)][int(self.my_indexes_movement[int(agent)])]
            else:
                my_current_locations[int(agent)] = self.current_locations[int(agent)]
        
        for agent in self.agents:
            valid_actions = calculate_valid_space(self.final_locations[int(agent)],self.current_locations[int(agent)],self.moves_remaining[int(agent)])
            obs = {
                "action_mask": valid_actions,
                "observations": {'paths': self.Paths, 'initial locations': self.initial_locations,'current locations': my_current_locations,'final locations': self.final_locations,'paths ready': self.paths_ready}
            }
        ...
        # print(my_mask)
        # print(self.moves_remaining,self.current_locations,self.final_locations)
        ...
        if self.paths_ready:
            for i,a in enumerate(self.agents):
                if self.myflag or self.time_limit_elapsed:
                    rewards[a] = -100000000
                    truncations = {agent: True for agent in self.agents}
                else:
                    rewards[a] = 100-1*self.time_elapsed[i]
                    if self.time_elapsed_total >= 1000:
                        rewards[a] = rewards[a] + 10000
