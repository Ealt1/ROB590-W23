...
class CustomEnvironment(ParallelEnv):
    ...
    def reset(self, seed=None, return_info=False, options=None):
        ...
        self.ws = self.eng.create_workspace(self.numRows,self.numCols,self.obstacles)
        
        self.Agents = self.eng.cell(1,self.N)
    ...
    def step(self, actions):
        ...
        for i in range(0,self.N):
            self.Agents[i] = self.eng.agent(i+1,self.Paths,1)
        self.eng.plot_ws(self.ws, self.initial_locations, self.final_locations, self.Paths,nargout=0)
        # print(self.eng.get(self.Agents[0]))
        # self.eng.createBottlesSharedWith(self.Agents[2],self.Agents,nargout=0)
        for i in range(0,self.N):
            self.eng.createBottlesSharedWith(self.Agents[i],self.Agents,nargout=0)
            for j in range(self.N):
                flag = self.eng.getfield(self.Agents[j],'myflag')
                if flag == 1:
                    self.myflag = 1
        if self.eng.workspace['cycle_method'] == 'R2':
            self.eng.find_rainbow_cycles_all(self.Agents,self.Paths,2,nargout=0)
        else:
            for i in range(0,self.N):
                self.eng.findDrinkingSessions(self.Agents[i],self.eng.workspace['cycle_method'])
        self.eng.workspace['myflag'] = self.eng.set_initial_conditions(self.Agents,nargout=1)
        if self.eng.workspace['myflag'] == 1:
            self.myflag = 1

        if not self.myflag:
            self.random_order = self.eng.randperm(self.N)
            while np.sum(self.runs_completed) < self.N:
                for m in range(0,self.N):
                    ...
                    if not self.runs_completed[n]:
                        ...
                        self.eng.move_philosopher(self.Agents[n],nargout=0)
                        
                        self.time_elapsed[n] = self.time_elapsed[n]+1
                        if self.eng.getfield(self.Agents[n],'curr_pos_idx') == self.eng.length(self.eng.getfield(self.Agents[n],'path')):
                            self.runs_completed[n] = 1
                for n in range(self.N):
                    curr_pos_idx_field = self.eng.getfield(self.Agents[n],'curr_pos_idx')
                    mypath = self.eng.getfield(self.Agents[n],'path')
                ...
                self.eng.plot_ws(self.ws, self.positions, self.final_locations, [],nargout=0)
                self.eng.drawnow
